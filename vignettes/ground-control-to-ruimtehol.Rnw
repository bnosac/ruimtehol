...
Now for the second use case: \textbf{transfer learning}. What we will do as an example is build a simple wordspace model to get word embeddings and pass these embeddings on as starting values for anoather tagspace model.
...
It's important to note that we need to pass on pretrained embeddings \textbf{for all terms as well as all labels which we want to keep training upon}. Starspace uses the default prefix \_\_label\_\_ for identifying a label, which we need to add as prefix to the label embeddings. As we haven't got the embeddings of the labels, in the example below, we assign some random staring values to the embedding of the labels. (Small note: in business settings it makes sense to let these labels start from a sensible combination of the embeddings of words which you think are similar to that label).
...
<<>>=
plot(model)
starspace_knn(model, "__label__FISCALITEIT")
@
